...
assert API_URL != None, "Error: 'INFOBIP_APIURL env variable not set'"
assert API_KEY != None, "Error: 'INFOBIP_APIKEY env variable not set'"

HEADERS = {
    'Authorization': API_KEY
}

PERSON_URL = "people/2/persons"
...
def get_persons(logger: Logger, filter: str | None) -> Iterator[dict]:
    logger.debug("Buscando personas")

    page = 1
    while True:
        url = f"{API_URL}/{PERSON_URL}?filter={filter}&page={page}&limit=500" if filter != None else f"{API_URL}?page={page}" 
...
def search_person(logger: Logger, phone: str) -> None | Lead:
    res = requests.get(f"{API_URL}/{PERSON_URL}?type=PHONE&identifier={phone}", headers=HEADERS)
    try:
        if not res.ok:
            logger.error("Error en la request: " + str(res.status_code))
            logger.error(res.json())
...
def update_person(logger: Logger, id: int, payload: dict):
    ...
    try:
        res = requests.put(f"{API_URL}/{PERSON_URL}", params={"id": id}, json=payload, headers=HEADERS)
        if not res.ok:
            logger.error("Error en la request: " + str(res.status_code))
            logger.error(res.json())
            return False
...
def add_fecha_lead(logger: Logger, leads: list[Lead]) -> bool | list[dict]:
    ...
    try:
        res = requests.patch(f"{API_URL}/{PERSON_URL}/batch", json=payload, headers=HEADERS)
        if not res.ok:
            logger.error("Error en la request: " + str(res.status_code))
            logger.error(res.json())
            return False
...
def create_person(logger: Logger, lead: Lead):
    ...
    try:
        res = requests.post(f"{API_URL}/{PERSON_URL}", json=payload, headers=HEADERS)
        if not res.ok:
            logger.error("Error en la request: " + str(res.status_code))
            logger.error(res.json())
